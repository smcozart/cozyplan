...
def render_index_html(plans: list[dict], dangling: list) -> str:
...
<body><main>
  ...
  <footer>Generated {esc(now_iso())} by plan_tool index. Derived artifact — do not hand-edit.</footer>
</main></body></html>
"""
...
def cmd_index(args) -> int:
    ...
    (specs / "_index.json").write_text(
        json.dumps({"generated": now_iso(), "plans": plans, "dangling": dangling},
                   indent=2, ensure_ascii=False),
        encoding="utf-8",
    )
    (specs / "_index.html").write_text(render_index_html(plans, dangling), encoding="utf-8")
...
def cmd_roles(args) -> int:
    ...
    manifest = {"generated": now_iso(), "roles": roles_out}
...
<body><main>
  ...
  {accomplishments}
  <footer>Generated {esc_(now_iso())} by plan_tool rollup. Derived artifact — do not hand-edit.</footer>
</main></body></html>
"""
...
def cmd_rollup(args) -> int:
    ...
    for pf in sorted(all_plans):
        ...
        if ts and (now - ts).days > STALE_DAYS:
            stale.append({"plan": pf, "reason": f"no report in {(now - ts).days}d"})

    out = {"generated": now_iso(), "role_filter": role_filter, "attention": attention,
