...
# dependencies = []
# ///
"""PostToolUse lint: validate a plan after any write to specs/*.html.

Reads the hook payload from stdin. If the tool touched a specs/*.html file
(Edit/MultiEdit/Write by path, or a Bash command mentioning one), it runs
plan_tool validate and, on failure, feeds the problems back to the agent as
additionalContext so it can self-correct. Fail-open on any unexpected error.
"""
