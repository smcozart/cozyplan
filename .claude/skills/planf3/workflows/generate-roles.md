...
6. **Author the team map.** Using the `excalidraw-diagram` skill (same tooling as plan diagrams — see `workflows/diagram-generation.md`), draw the role structure: one box per role, `reports_to` arrows, and labeled links for any seam contracts between lanes. Keep it minimal (boxes, arrows, short labels; match the plan palette). Save as `roles/team-map.excalidraw` and render to `roles/team-map.png`. `PLAN_TOOL rollup` embeds the PNG at the top of `specs/_status.html` automatically when it exists. If the excalidraw skill is unavailable, skip — the rollup degrades gracefully. Re-author whenever roles are added, split, or re-wired.
7. **Assign owners to plans.** For each existing/new plan, set `PLAN_TOOL meta <plan> --field owner --value <role>`.
8. **Report.** Summarize the role set and ownership map back to the user, and note that each agent/human assuming a role should follow the role file's Session bootstrap (set `PLANF3_ROLE`).
