...
2. **Map ownership globs.** For each role propose `source_of_truth`, `code`, and `supporting` globs. Source-of-truth and code globs MUST be disjoint across roles (overlap is what causes merge pain — `roles build` will reject it). Shared files (README, root configs) stay unowned and architect-gated.
3. **Confirm with the user (interview step).** Use AskUserQuestion to present the proposed role set and ownership map; let the user add / merge / split / rename roles and correct globs. This is the "who is responsible for what" sign-off before any building starts.
4. **Write role files.** For each role, author `roles/<role>.md` from `templates/role.md`, filling mission, responsibilities, and a concrete Definition of Done. Create an empty `roles/<role>/memory.md` for each.
