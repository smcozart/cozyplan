...
3. Link Both Sides - For each related plan, run `PLAN_TOOL ref --this TARGET.html --other OTHER.html --dir back|forward`. One command handles everything: it adds the reference to the target, adds the reciprocal on the other plan (so links stay bidirectional), stamps `modified` on both, and records an amendment on each. It dedupes, so re-running is safe. (If the other plan lacks anchors, run `PLAN_TOOL init-ids OTHER.html` first.)
