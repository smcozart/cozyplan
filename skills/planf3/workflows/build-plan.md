...
3. Execute Phases - For each phase in order, top to bottom:
   ...
   - On phase completion, commit the phase's work with git (and optionally tag it, e.g. `git tag planf3/<id>/phase-<n>`). Commits and tags are your revert points — planf3 records intent, git owns checkpoints and reverts
4. Final Validation - Run the global Validation Commands, mark each with `PLAN_TOOL status`, and confirm every box is `[x]` or `[f]`
5. Update Metadata - Record the build in the metadata header: append `PLAN_TOOL meta PLAN_FILE --field agent --value <name>`, `--field session --value <id>`, and `--field commits --value <sha>` for the build commit(s) (each appends; `modified` is stamped automatically on every CLI write). When the plan is implemented and tests pass, set `PLAN_TOOL meta PLAN_FILE --field status --value built`
6. Report - Summarize what was built per phase, the final status of every task, and any `[f]` failures that need attention. Acceptance is not a planf3 step: open a PR and let review + merge (git, CODEOWNERS, CI) be the acceptance gate
