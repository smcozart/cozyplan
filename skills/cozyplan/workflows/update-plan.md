...
4. Update Metadata - Append the current ISO timestamp to `modified` and append the agent name / session id to their lists
5. Record Amendment - Append a new entry to the Amendments section (newest at the bottom) summarizing what changed and why
6. Sync State - If `STATE_FILE` exists, run `workflows/sync-state.md` (ledger entry; refresh the plan's In Development row if its scope or status changed)
