...
3. **Append events** - One `PLAN_TOOL state add` per change. `--kind claim` with `--proof`, `--sha`, and `--paths` (paths are what let `state check` report that a claim's own code moved, so a claim without them is checkable only by commit distance); `--kind indev` with `--status` and `--owner`; `--kind gap` for what is known-broken or unverified. Supersede an entry by adding a new event with the same `--key`; retract one with `--clear`. Carry the ids: `--plan`, `--phase`, `--adr`, `--issue`, `--session`.
