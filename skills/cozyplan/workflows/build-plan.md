...
2. Absorb Context - Recall is **index, not store**. Read the plan's state cheaply, then pull detail only for the phase in hand:
...
3. Resume - Assume nothing about how far a prior session got; a build is as likely to be a re-entry as a fresh start:
   - Run `PLAN_TOOL next PLAN_FILE` to get the re-entry point — it prints the first non-terminal id (the first marker that is not `[x]` or `[f]`), or `done`. Take that id mechanically
   - `done` means every marker is terminal — skip to Final Validation and Report; rebuild nothing
   - Phases and tasks marked `[x]` are complete: skip them entirely
...
4. Execute Phases - Start at the phase `next` resolved to and work forward in order, top to bottom:
   ...
   - Finish the current phase's tasks and tests before starting the next phase
...
6. Update the Component Map - If this build added, removed, renamed, or re-owned a component — or created or changed a contract that crosses a process, repo, or network boundary — update its row in `SYSTEM.md` (nodes: name · responsibility · owner · why-links; edges: from · to · kind · the literal contract string · what breaks — point the why-link at the very plan/ADR just built; the map's format and rules live in the sibling **`discuss`** skill's `templates/system.md`). Skip silently when the project has no `SYSTEM.md` or nothing structural changed
...
8. Sync State - If `STATE_FILE` exists, run the `workflows/sync-state.md` subworkflow: append a `claim` event for each validated result (the passing Validation Commands are its proof, with `--paths` naming what it depends on), update the In Development entries this build touched, and append the ledger entry. Each `[f]` failure becomes a work item — `PLAN_TOOL issue file --title "…" --plan <id>`, which queues when `gh` is away.
9. Report - Summarize what was built per phase, the final status of every task, and any `[f]` failures that need attention. Acceptance is not a cozyplan step: open a PR and let review + merge (git, CODEOWNERS, CI) be the acceptance gate

**Completion criterion:** `PLAN_TOOL next PLAN_FILE` prints `done`, every `[f]` is filed as a work item with the reason `brief` recorded, the plan's `status` is `built` (or the user has explicitly accepted an incomplete build), and `PLAN_TOOL state check` exits 0.
