...
1. Detect - Check for `STATE_FILE`, `docs/state.ndjson`, `docs/adr/`, `docs/journal.md`, and `docs/agents/issue-tracker.md`. If all exist and are populated, report that the state layer is already initialized and stop. If partial, create only what is missing.
