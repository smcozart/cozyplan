...
| State layer | Command |
| --- | --- |
| Append an event | `PLAN_TOOL state add --kind claim\|indev\|gap --what "…" [--proof "…"] [--sha X] [--paths a,b] [--adr N] [--issue N] [--plan P] [--clear]` |
| Rebuild `STATE_FILE` from the log | `PLAN_TOOL state render [--origin origin/main] [--project NAME] [--dry-run] [--force]` |
...
Both registration paths — the plugin manifest and `PLAN_TOOL hooks install` — write the same command, through the same `run-hook.sh`. When the scripts live inside the project (vendored, or a source checkout) the registration is written against `${CLAUDE_PROJECT_DIR}`, so a committed `.claude/settings.json` stays correct on a teammate's clone. `--global` keeps absolute paths, since that file is shared across projects.
