...
## [2026-08-24T18:36:32-05:00] Sean Cozart <seancozart@outlook.com> · via Claude Opus 5 · session s-enforce-01
- **What:** `plan_tool.py` (ADR register check, `resolve_git_hook_tool`, doctor's `git hook tool` row), `tests/test_state_check.py`, `tests/test_trailers.py`, `docs/handoff-from-cozycode-step-3.md`
- **Why:** cozycode ran step 3 and reported back two defects in cozyplan, both the shape ADR-0010 exists to catch, both inside checks written to catch it. `state check` compared the ADR register against the same glob `render_state` generates it from, so it could not fail — `OK STATE.md: consistent with git` for an ADR git had never seen. And `hooks git-install` recorded whichever `plan_tool` invoked it, so cozycode's git hooks ran *this* checkout to answer whether cozycode was healthy.
- **State →** 273 tests pass; `doctor --strict`, `hooks selftest --shipped` and `state check` all exit 0. The register check now reads the git index while render still reads the disk — the asymmetry is what keeps the disagreement representable. `git-install` prefers an in-repo copy and records it relatively; `doctor` gained a `git hook tool` row naming which tool the hooks run. Both fixes mutation-checked. cozycode also corrected this repo's §4 structural claim: `swimschool` was created *during* the session that read the handoff and held none of the studied defects — three of four lived in `reference`, which has CI, so the "no CI tier to move from" conclusion inverted. Commit a03f28e.
- **Refs:** ADR-0010, cozyplan#4, cozycode#16, cozycode ADR-0012

## [2026-08-24T18:59:56-05:00] Sean Cozart <seancozart@outlook.com> · via Claude Opus 5 · session s-enforce-01
- **What:** `skills/cozyplan/templates/state-check.yml`, `plan_tool.py` (doctor `ci workflow` + new `ci runs selftest` rows), `tests/test_doctor.py`
- **Why:** Close the two gaps cozycode reported after step 3. One was worse than recorded: `hooks selftest` had been added to this repo's own workflow and never to the template `init` installs, so the gap covered new repos too, not just already-wired ones.
- **State →** 277 tests pass; `doctor --strict`, `hooks selftest --shipped` and `state check` all exit 0. A freshly inited repo now gets the selftest step; an already-wired one gets a `ci runs selftest` warn naming the exact command, which cozycode reports today. The `ci workflow` row is labelled a record — it greps YAML for a string, so a broken or never-green workflow passed it identically to one guarding every push. Commit df0acd5.
- **Refs:** ADR-0010, cozyplan#4, cozycode#16

## [2026-08-24T19:10:40-05:00] Sean Cozart <seancozart@outlook.com> · via Claude Opus 5 · session s-enforce-01
- **What:** `plan_tool.py` (`_vendored_freshness`, VENDORED.md now records the source remote), `tests/test_doctor.py`; and in cozycode, a re-vendor committed as 83dbbc2
- **Why:** Version skew bit three times in one session and each time presented as something else — a rehearsal against a copy predating its own fix, and cozycode twice carrying a plan_tool without the fixes it had reported. A snapshot pinned at a commit does not change when upstream moves, so stale and current were indistinguishable from inside the repo.
- **State →** 282 tests pass; all three gates exit 0. `doctor` now reports how many commits a vendored copy is behind, and names what it could not check rather than implying freshness — the unreachable, wrong-repo and no-provenance branches each get their own warn. cozycode was re-vendored first (rehearsed on a clone at its exact HEAD): four files, all under `.claude/`, its own 52-line `.githooks/pre-commit` untouched, and `cozyplan.plantool` now points at its own in-repo copy instead of this checkout. Its gate was verified to still refuse — an ADR rendered into Registers and never staged stops the commit, which closes the blind spot cozycode's ADR-0012 recorded. Commit 50fde96.
- **Refs:** ADR-0010, cozyplan#4, cozycode#16, cozycode ADR-0012

## [2026-08-24T21:56:07-05:00
- the concept is called sync in STATE.md's 'Last synced' header and in doctor's prose, but 'state sync' does not exist — it is state add + state render. Alias it or stop calling it sync (reported by cozycode)] Sean Cozart <seancozart@outlook.com> · via Claude Opus 5 · session s-enforce-01
- **What:** `plan_tool.py` (`vendor_source_problem`, refusal wired into `cmd_init`), `tests/test_init.py`, `docs/cozycode-next-steps.md`
- **Why:** cozycode re-vendored, then reported a bug it verified and deliberately did not fix: the "a vendored copy must not perform its own upgrade" guard existed only in my prose. `_vendored_freshness` had one call site — `doctor` — and `cmd_init` never consulted it.
- **State →** 300 tests pass; all three gates exit 0. `init --vendor` now refuses to run from a vendored plan_tool, before anything is written. Reproducing their case found a second, destructive variant: same root means source and destination are one directory, so the rmtree clearing the destination deleted the source — 21 files gone before FileNotFoundError. Their suggested shape (refuse when plan_tool is inside --root) would not have caught their own repro; the load-bearing signal is that the running tool is itself vendored. Mutation-checked. Also recorded three friction items they reported, and rewrote the handoff, which had gone stale twice and carried four unverified numbers about their repo. Commit 8ff1913.
- **Refs:** ADR-0010, cozycode db8d438

## [2026-08-24T23:02:19-05:00
- the concept is called sync in STATE.md's 'Last synced' header and in doctor's prose, but 'state sync' does not exist — it is state add + state render. Alias it or stop calling it sync (reported by cozycode)] Sean Cozart <seancozart@outlook.com> · via Claude Opus 5 · session s-enforce-01
- **What:** `docs/state.ndjson`, `STATE.md` — seven claims re-anchored, no code change
- **Why:** CI went red on the two documentation commits. Not the docs: this session added enough commits to push seven claims past `state check --max-claim-age 50`, and the last two runs were the first to cross it. The check was working — those proofs had not been re-run in 52 commits.
- **State →** Each of the seven proofs was actually re-run before its claim was re-anchored, not re-stamped: the full suite (300 passed), `test_init.py` (31), `test_state_migrate.py` (12), `test_trailers.py` (20), `test_doctor.py` + `test_state_check.py` (49), `test_issue.py` (12), and plan validation with `uv` removed from PATH entirely (3 validated, 0 failed). `state check --max-claim-age 50` now exits 0. Worth recording that the failure was invisible locally — bare `state check` passes, and only the flag CI actually uses shows it, which is why the local gate and the CI gate must run the same command.
